...
export default async function Ads() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <form action="/api/admin/ads" method="post" className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-4">
        ...
        <input name="targetUrl" placeholder="Target URL" className="rounded border px-3 py-2" />
        <button className="rounded bg-black px-4 py-2 text-white">Add</button>
      </form>
      <ul className="mt-6 divide-y">
        {items.map((i) => (<li key={i.id} className="py-2">{i.position} • <a href={i.targetUrl} className="text-blue-600 hover:underline">{i.targetUrl}</a></li>))}
      </ul>
    </div>
  );
}
